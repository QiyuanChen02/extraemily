	...
	return (
		<div className={`absolute min-w-full min-h-screen pb-16 bg-colour`}>
			<Head>
				...
				<meta
					name="description"
					content="A website for the Twitch streamer ExtraEmily"
				/>
				...
				<meta
					property="og:url"
					content="https://extraemily.vercel.app"
				/>
				<meta
					property="og:image"
					content="http://extraemily.vercel.app/_next/image?url=%2Flogo.png&w=2048&q=75"
				/>
			</Head>
			...
			<Navbar>
				...
				<Section type="NavbarLinks" display={!loading}>
					{isSmall ? (
						...
						<>
							{socials.map((social) => (
								<SocialIcon media={social} key={social} />
							))}
						</>
					)}
				</Section>
			...
			{drawerOpen && (
				<Drawer closeModal={closeModal}>
					{socials.map((social) => (
						<SocialIcon media={social} key={social} hasText />
					))}
				</Drawer>
			)}
...
export async function getStaticProps() {
	...
	const youtubeclipData = await fetchData<YoutubeclipData>(
		`${server}/api/youtubeclip`
	)
